...
try:
    ...
    # Try to load the hand landmarker model
    try:
        ...
        mp_drawing = None  # Not used in new API
    except Exception as e:
...
def recognize_gesture_mediapipe(image, alphabet_mode=False):
	...
	if detection_result.hand_landmarks:
		...
		if len(detection_result.hand_landmarks) == 2:
			left_hand_closed = detect_hand_closed(detection_result.hand_landmarks[0])
			right_hand_closed = detect_hand_closed(detection_result.hand_landmarks[1])
		...
		for hand_landmarks in detection_result.hand_landmarks:
			# Skip drawing for now with the new API
			
			# Check for alphabet gestures first if in alphabet mode
			if alphabet_mode:
				alphabet_gesture = detect_alphabet_gestures_mediapipe(hand_landmarks)
				if alphabet_gesture:
					return alphabet_gesture, 0.9
			
			# Count fingers
			finger_count = count_fingers_mediapipe(hand_landmarks)
			
			# Check for special gestures
			special_gesture = detect_special_gestures_mediapipe(hand_landmarks)
